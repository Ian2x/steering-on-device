...
- The current residual-edit path computes a front-aligned per-position `h(A) - h(B)` matrix from unpadded contrast prompts, injects it across aligned prompt positions after the selected block, and bakes the edit into downstream KV caches.
- The orange and purple traces are `KL(candidate || baseline)` between the actual temperature-scaled sampling distributions.
- The app runs one of **two KL disciplines**, selected in the UI and recorded in every packet as `klDiscipline`. They are described below; the historical results in this README were produced under the cap.
...
### The two KL disciplines

**Greedy cumulative cap** is the historical rule, kept because every committed packet in this repository was produced under it and because the failure it produces is worth showing. One cumulative budget is spent first-come-first-served with no lookahead: each step takes as much divergence as it wants until the budget is gone, after which generation continues from the steered prefix with no further intervention. It is a **prefix intervention under a distributional cost ceiling**, not sustained steering. In the six documented logit-bias sanity rows the budget is exhausted over two to eighteen steps; the eighteen is a lone outlier, with a median of three and five of the six at five or fewer. The dense arm is far worse, and its first-step collapse is the confound described above. Both arms are capped under this discipline; earlier builds capped only the sparse arm, which is strictly worse than either symmetric choice.

**Matched per-step** is the default, and it is the discipline the audit actually used. There is no cap and no adaptive rescaling. A fixed scalar applies at every step, and the app's **Calibrate** button bisects that scalar per arm until the arm's *teacher-forced* mean KL per step lands within `0.002` of the audit's `0.43523801873284795` target. Calibration is a separate explicit action rather than something that happens during a steered run, because matching two arms on their **free-running** mean KL is not a match at all: the two means would be taken over different token sequences. Only a shared, fixed base-generated continuation makes them comparable, so that is what the button builds first and teacher-forces both arms through. The selection rule is Amendment 1's: minimum absolute tested mean-KL error, ties to the lower scalar. The search is [`CalibrationSelector`](SteeringKit/Sources/SteeringKit/CalibrationSelector.swift) in `SteeringKit`, shared by the app and the test suite, and it does not assume the KL curve is monotone in the scalar, because on the committed wedding residual calibration it was not.

Under this discipline the app also reports mean nats/step and exhausted-step counts per arm, which are the quantities that make the cap's collapse visible rather than implicit. The frozen protocol runners set the discipline themselves and are unaffected by the UI selector.

[`docs/kl-disciplines.md`](docs/kl-disciplines.md) has the rest: why the cap was wrong on level as well as on schedule, and why cap mode reproduces the historical schedule without reproducing the historical bytes.
...
`SteeringKit` has 17 tests, including hand-computed three-token KL cases, speculative-read-ahead accounting, randomized sparse- and nonlinear dense-candidate budget-bound checks, the coefficient-zero byte-identity path used by the app, and high-precision golden values generated independently with Python `decimal` through direct normalization plus entropy/cross-entropy. Five of them cover calibration: one replays all four committed teacher-forced calibration curves through the shared selector and requires the frozen scalars back, having never evaluated a scalar the original runner did not test, and the rest pin below-target selection, the non-monotone reversal, and both failure modes. `make verify-fixtures` regenerates and compares both committed fixtures. The Core ML export was compared with its PyTorch source on 24 inputs, including empty, non-Latin, near-limit, and truncated cases. The minimum cosine agreement was `0.999914432`, above the `0.9999` gate; the report also records maximum absolute delta, relative L2 error, both embedding norms, compute units, and the model-weight SHA-256. See [`docs/coreml-parity.md`](docs/coreml-parity.md) and the raw [`docs/coreml-parity.json`](docs/coreml-parity.json).
...
The audit did **not** use a greedy cumulative cap. In its audited cell, the target was a teacher-forced mean KL of `0.43523801873284795` nats per step, sustained over a fixed 64-step continuation. Every committed result in this repository predates the matched-per-step discipline and was produced under the cap; the app's new default follows the audit's rule, but running it does not retroactively make any packet here comparable to the audit. Its ActAdd direction was injected at prompt positions 0 through 6 and baked into the KV cache at layer 20 of 48; the protocol also included a matched-norm random-direction floor and repetition, length, and NLL degeneracy gates. Those choices and this app's historical Phase 6 protocol are not equivalent.
